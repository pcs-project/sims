...
public class DisabledIdCardNoGenerator extends SequenceStyleGenerator{
		...
		public Serializable generate(SharedSessionContractImplementor session,
			...
			return String.format(dateFormat, LocalDate.now(), super.generate(session, object))   + DATE_NUMBER_SEPARATOR_DEFAULT +
					((DisabledIdCard)object).getDisabledAddressDetails().getDistrict() +DATE_NUMBER_SEPARATOR_DEFAULT + ((DisabledIdCard)object).getDisabledAddressDetails().getMunicipality()+DATE_NUMBER_SEPARATOR_DEFAULT+	 ((DisabledIdCard)object).getIdCardType()+ dateNumberSeparator+ 
