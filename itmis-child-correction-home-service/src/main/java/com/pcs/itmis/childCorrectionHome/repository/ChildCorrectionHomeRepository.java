...
public interface ChildCorrectionHomeRepository  extends JpaRepository<ChildCorrectionHome, Long>{
  ...
  @Query("select max(c.synchronizedDate) from ChildCorrectionHome c ")
  LocalDate findMaxSynchronizedDate();
  
  @Query(value ="select c from ChildCorrectionHome c where c.entryDate between :fromDate and :toDate  and c.synchronizedDate is null")
  List<ChildCorrectionHome> findAllBetweenFromAndToDate(LocalDate fromDate, LocalDate toDate);
