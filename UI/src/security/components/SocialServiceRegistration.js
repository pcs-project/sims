...
const SocialServiceRegistration = () => {
  ...
  const [serviceTypeList, setServiceTypeList] = useState([
    {
      value: "counselling",
      engName: t("counselling"),
    },
    {
      value: "medical",
      engName: t("medicalFacilities"),
    },
    {
      value: "lifeskill",
      engName: t("lifeSkill"),
  ...
  const [sourceOfFundingList, setSourceOfFundingList] = useState([
    ...
    {
      value: "governmentFund",
      engName: t("governmentFund"),
    },
    {
      value: "buySellAssets",
      engName: t("buySellAssets"),
    },
    {
      value: "communityBased",
      engName: t("communityBased"),
    },
    {
      value: "others",
      engName: t("others"),
  ...
  const {
    ...
    isOwnBuilding: TRUE_FALSE.FALSE,
    isRented: TRUE_FALSE.FALSE,
  });
    ...
    SocialServiceRegistrationService.getAllSocialServiceRegistration().then((response) => {
      ...
      response.data.data.forEach((socialService) => {
        AddressService.getProvinceByProvinceId(socialService.province).then((response) => {
          socialService.provinceName = response.data.data.provinceDescEng;
        });
        AddressService.getDistrictbyProvinceCdAndDistrictCd(
          socialService.province,
          socialService.district
        ).then((response) => {
          socialService.districtName = response.data.data.districtDescEng;
          index += 1;
  ...
  const onSubmit = (data) => {
    ...
    buttonLabel == "Update"
      ? trackPromise(
          SocialServiceRegistrationService.updateSocialServiceRegistration(
            data.registrationId,
            data
          )
            .then((response) => {
      ...
      : trackPromise(
          SocialServiceRegistrationService.saveSocialServiceRegistration(data)
            .then((response) => {
              if (response.status === 200) {
                toast.current.show({
                  severity: "success",
  ...
  const acceptUpdate = (socialService) => {
    ...
    reset({
      ...
      physicalInfrastructure: socialService.physicalInfrastructure,
      isRented: socialService.isRented,
  ...
  const acceptDelete = (socialService) => {
    SocialServiceRegistrationService.deleteSocialServiceRegistration(
