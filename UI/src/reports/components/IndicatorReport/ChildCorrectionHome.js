...
const ChildCorrectionHome = () => {
  ...
  const searchData = (data) => {
    ...
    console.log(data);
    trackPromise(
      IndicatorReportService.getChildCorrectionHomeData(data).then(
        (response) => {
          console.log("child correction report", response.data);
  ...
  const exportToCsv = () => {
    ...
    var check = document.getElementById('check').innerHTML;
		if(check==="क्र.स.")
		{
			...
			let ctx = { worksheet: 'Child Correction Home report.xls' || 'Worksheet', table: table1}
			// window.location.href = uri + base64(format(template, ctx));
			downloadLink.href = uri + base64(format(template, ctx));
			downloadLink.download = ('Child Correction Home report'||"exportedTable") + ".xls";
		...
		else
		{
			...
			// Download it
			CsvExport.csvDownload(csv, 'Child Correction Home report.csv');
		}
  }
              ...
              <div className="section-to-print">
                ...
                <table>
                  ...
                  <tbody>
                    <tr>
                      <th>{t("1")}</th>
                      <th className="main-indicator">{t("totChildCorrectionHome")}</th>
                      <td>{responseData.tot_child_correction_home}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th>{t("2")}</th>
                      <th className="main-indicator">{t("totChildReceivingServicesInCch")}</th>
                      <td>-</td>
                      <td>{responseData.boys_receiving_services_in_cch}</td>
                      <td>{responseData.girls_receiving_services_in_cch}</td>
                      <td>{responseData.others_receiving_services_in_cch}</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("dalit")}</th>
                      <td>{responseData.dalit_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("muslim")}</th>
                      <td>{responseData.muslim_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("janajati")}</th>
                      <td>{responseData.janajati_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("madhesi")}</th>
                      <td>{responseData.madhesi_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("brahminOther")}</th>
                      <td>{responseData.brahmin_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("ethnicMinorities")}</th>
                      <td>{responseData.ethnic_minorities_receiving_services_in_cch}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th>{t("3")}</th>
                      <th className="main-indicator">{t("totChildRehabilitatedFromCch")}</th>
                      <td>-</td>
                      <td>{responseData.boys_rehabilitated_from_cch}</td>
                      <td>{responseData.girls_rehabilitated_from_cch}</td>
                      <td>{responseData.others_rehabilitated_from_cch}</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("home")}</th>
                      <td>{responseData.rehabilitated_from_home}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("ojt")}</th>
                      <td>{responseData.rehabilitated_from_ojt}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("outOfCommunity")}</th>
                      <td>{responseData.rehabilitated_from_out_of_community}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("adoption")}</th>
                      <td>{responseData.rehabilitated_from_adoption}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("fosterHome")}</th>
                      <td>{responseData.rehabilitated_from_foster_home}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th>{t("4")}</th>
                      <th className="main-indicator">{t("perBudgetForChildCorrectionHome")}</th>
                      <td>{responseData.budget_for_child_correction_home}%</td>
                      <td colspan={3}>-</td>
                    </tr>
                  </tbody>
                </table>
...
export default ChildCorrectionHome;
