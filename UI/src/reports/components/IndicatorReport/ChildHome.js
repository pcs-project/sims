...
const ChildHome = () => {
  ...
  const searchData = (data) => {
    ...
    console.log(data);
    trackPromise(
      IndicatorReportService.getChildHomeData(data).then((response) => {
        console.log("child home report", response.data);
  ...
  const exportToCsv = () => {
    ...
    var check = document.getElementById('check').innerHTML;
		if(check==="क्र.स.")
		{
			...
			let ctx = { worksheet: 'Child Home report.xls' || 'Worksheet', table: table1}
			// window.location.href = uri + base64(format(template, ctx));
			downloadLink.href = uri + base64(format(template, ctx));
			downloadLink.download = ('Child Home report'||"exportedTable") + ".xls";
		...
		else
		{
			...
			// Download it
			CsvExport.csvDownload(csv, 'Child Home report.csv');
		}
  }
    ...
    <>
      ...
      >
        <div className=" p-card-content">
          <h4 className="p-pt-0">{t("childHomeIndicator")}</h4>
        </div>
      </Card>
              ...
              <div className="section-to-print">
                ...
                <table>
                  ...
                  <tbody>
                    <tr>
                      <th>{t("1")}</th>
                      <th className="main-indicator">{t("totChildHome")}</th>
                      <td>{responseData.tot_child_home}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th>{t("2")}</th>
                      <th className="main-indicator">{t("totChildReceivingServices")}</th>
                      <td>-</td>
                      <td>{responseData.boys_receiving_services}</td>
                      <td>{responseData.girls_receiving_services}</td>
                      <td>{responseData.others_receiving_services}</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("tip")}</th>
                      <td>{responseData.tip_receiving_services}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("gbv")}</th>
                      <td>{responseData.gbv_receiving_services}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("trappedInAddiction")}</th>
                      <td>{responseData.trapped_in_addiction_receiving_services}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("drugAddict")}</th>
                      <td>{responseData.drug_addict_receiving_services}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                      <th className="sub-indicator">{t("streetChildren")}</th>
                      <td>{responseData.street_children_receiving_services}</td>
                      <td colspan={3}>-</td>
                    </tr>
                    <tr>
                      <th></th>
                    ...
                    <tr>
                      <th>{t("3")}</th>
                      <th className="main-indicator" colspan={5}>{t("totChildRehabilitated")}</th>
                    </tr>
                    <tr>
                      <th></th>
                    ...
                    <tr>
                      <th>{t("4")}</th>
                      <th className="main-indicator">{t("perBudgetForChildHome")}</th>
                      <td>{responseData.budget_for_child_home}</td>
                      <td colspan={3}>-</td>
                    </tr>
                  </tbody>
                </table>
